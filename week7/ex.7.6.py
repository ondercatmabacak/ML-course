...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 20 14:24:40 2019
...
for n_cluster in result.keys():
    ...
    for score_type in result[n_cluster]["scores"].keys():
        score = result[n_cluster]["scores"][score_type]
...
sil_list = [result[n]["scores"]["sil_score"] for n in result.keys()]
...
lscore_list = [result[n]["scores"]["lscore"] for n in result.keys()]
...
ax1.scatter(n_cluster_list, lscore_list, color="tab:blue", label="avg log likelihood score")
ax1.plot(n_cluster_list, lscore_list, color="tab:blue")
...
n_cluster_list = result.keys()
# get AIC score list
aic_score_list = [result[n]["scores"]["aic_score"] for n in result.keys()]
...
ax1 = plt.gca()
# plot AIC  score 
ax1.scatter(n_cluster_list, aic_score_list, color="tab:blue", label="AIC score")
ax1.plot(n_cluster_list, aic_score_list, color="tab:blue")

# edit labels and other staffs
ax1.set_xlabel("n_clusters", size=14)
ax1.set_ylabel("AIC score", size=14)
ax1.legend(loc="best")
ax1.set_xticks(n_cluster_list)
ax1.legend(loc='upper left')
ax1.grid(axis="x")
...
n_cluster_list = result.keys()
# get BIC score list
bic_score_list = [result[n]["scores"]["bic_score"] for n in result.keys()]
...
ax1 = plt.gca()
# plot BIC scores 
ax1.scatter(n_cluster_list, bic_score_list, color="tab:blue", label="BIC score")
ax1.plot(n_cluster_list, bic_score_list, color="tab:blue")
...
gmm = result[best_n_clusters]["model"]
# get scores
scores = result[best_n_clusters]["scores"]
